<template>
  <div class="edit-post">
    <xheader title="تعديل المنشور" description="من هنا يمكنك عمل اي تعديل ترغب بيه في المنشورات.." />
    <div class="container">
      <div class="row">
        ...
        <form class="col s12" method="post" action=""  @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
          ...
          <div class="center-align">
            <button class="btn pulse" :disabled="form.errors.any()">
              <span><i class="material-icons">add</i>تعديل</span>
            </button>
          </div>
        </form>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      onSubmit() {
        this.loading = true;
        Post.update(this.id,this.form.data())
          .then((res) => {
            this.loading = false;
            if (res.status === 500) {
